...
C-----------------------------------------------------------------------
      ...
      CALL W3TAGB('BUFR_BUOYDUMP',2013,035,50,'NP20')
 
      print *
      print * ,'-> Welcome to BUFR_BUOYDUMP of 2/04/2013',
...
1     DO WHILE(IREADMG(LUBFR,NCOSET,IDATE).EQ.0)
      ...
      if(subset.eq.'        ') then
         ...
         print*,ncoset,' >>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<'
         print*,'buoydump error - unknown input subset encountered'
         CALL W3TAGE('BUFR_BUOYDUMP')
         call errexit(99)
      endif
...
C  -------------------------------------------------------
      ...
      if(nems.eq.0) then
         ...
         print*,subset,' >>>>>>>>>>>>>>>>><<<<<<<<<<<<<<<<<<<<'
         print*,'buoydump error - modsbufr subset missing definitions'
         CALL W3TAGE('BUFR_BUOYDUMP')
         call errexit(99)
      endif
